...
export const EDUCATION_HIERARCHY = {
    ...
    'POST GRADUATION': 6,
    'GRADUATION': 5,
    'DIPLOMA / ITI (POLYTECHNIC, ITI, DPHARM, PGDCA)': 4,
    'DIPLOMA': 4,
    // With space after parenthesis
    '(12TH) HIGHER SECONDARY': 3,
    '(10TH) SECONDARY': 2,
...
export const getRequiredEducationLevels = (highestLevel) => {
    ...
    const orderedLevels = [
        { key: 'graduation', value: 5 },
        { key: 'diploma', value: 4 },
        { key: '12th_higher_secondary', value: 3 },
        { key: '10th_secondary', value: 2 }
    ];
    
    for (const level of orderedLevels) {
        if (level.value <= hierarchyValue && level.value >= 2) {
...
export const getUserEducationLevels = (userHighestLevel) => {
    ...
    const orderedLevels = [
        { key: 'post_doctorate', value: 8 },
        { key: 'phd', value: 7 },
        { key: 'post_graduation', value: 6 },
        { key: 'graduation', value: 5 },
        { key: 'diploma', value: 4 },
        { key: '12th_higher_secondary', value: 3 },
        { key: '10th_secondary', value: 2 }
...
export const checkMarksPercentageWithPwd = (userPercentage, examData, levelKey, casteCategory, isPwd) => {
    ...
    if (isPwd && examData?.pwd_status && typeof examData.pwd_status === 'object') {
        // PWD status object contains category-wise percentage requirements
        marksRequirement = examData.pwd_status;
        requirementSource = 'PWD';
    } else if (examData?.education_levels?.[levelKey]?.marks_percentage) {
        // Use regular marks percentage from education level
